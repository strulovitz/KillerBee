...
# Usage: autoinstall_one.sh <vm-name> <ram-MB>
# Example: autoinstall_one.sh giantqueen-b 12288
set -euo pipefail
...
echo "=== starting autoinstall for $NAME (RAM=${RAM_MB}M, disk=$DISK) ==="
sudo virt-install \
  --name "$NAME" \
  --memory "$RAM_MB" \
  --vcpus 2 \
  ...
  --disk "path=$DISK,format=qcow2,bus=virtio,size=15" \
