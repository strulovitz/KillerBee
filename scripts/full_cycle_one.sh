#!/bin/bash
# Phase 3: autoinstall + boot + swap-check + ollama install + verify, for one VM.
# Usage: full_cycle_one.sh <vm-name> <ram-MB> <disk-GB> <vcpus>
# Example: full_cycle_one.sh giantqueen-b 8192 80 6
set -euo pipefail

NAME="${1:?missing vm name}"
RAM_MB="${2:?missing ram MB}"
DISK_GB="${3:?missing disk GB}"
VCPUS="${4:?missing vcpu count}"
...
echo "====> $NAME: swap verification (V3 Revision 5)"
SWAP_OUT=$($SSH "nir@$IP" 'sudo swapon --show; echo "---"; free -h' 2>&1)
echo "$SWAP_OUT"
if ! echo "$SWAP_OUT" | grep -qiE '(/swap|partition|file)'; then
  echo "WARN: $NAME has no swap configured. Adding 2G swap file."
  $SSH "nir@$IP" 'sudo fallocate -l 2G /swap.img && sudo chmod 600 /swap.img && sudo mkswap /swap.img && sudo swapon /swap.img && echo "/swap.img none swap sw 0 0" | sudo tee -a /etc/fstab'
fi
