...
| M2 | Queens | tier-DQ/GQ | `qwen3.5:35b-a3b` (35B/3B active, "gold standard" per "medium-small MoE" reconfirmation, runs at 8-12 GB VRAM Q4), `qwen3:30b-a3b` (30B/3B active) | _pending_ |
| M3 | RajaBee | tier-RB | `glm-4.7-flash` (30B/3B active, thinking mode), `gemma4:26b-moe` (26B/3.8B active, 88.3% AIME math), `mixtral:8x7b` (47B/13B active, borderline at q3), `nemotron-cascade-2:30b` (30B/3B active, NVIDIA, competitive programming + STEM), `qwen3:30b-a3b-thinking` (30B/3B active, 262K context, reasoning variant), `qwen3-coder-next:32b` (32B coder variant), `nemotron-3-nano:30b` (hybrid MoE, dual-classified with D4 dense). Out-of-budget: deepseek-v3.2 671B, qwen3.5 397B-A17B, glm-5 744B, kimi-k2.5 1T, qwen3 235B-A22B, mixtral 8x22B, mistral-3-large 675B, **`gpt-oss:120b`** (117B/5.1B active, GPT-4-level claim, ~50 GB at q3 — too big for 18 GB tier even at q2, from "medium-big MoE"), **`llama4:scout`** (109B/17B active, ~50 GB at q3, from "medium-big MoE"). | _pending_ |
...
- **2026-04-14 afternoon** — `desktop-template` autoinstall completed. VM boots to login prompt, DHCP lease on `br0` = `10.0.0.9`, SSH via `~/.ssh/phase3_ed25519` works as user `nir`, hostname `desktop-template`, Ubuntu 24.04.4 LTS kernel 6.8.0-110-generic. `apt` base pkgs (curl/python3/python3-pip/git/ca-certificates) all present.
- **2026-04-14 afternoon** — Ollama installed in `desktop-template` via `curl -fsSL https://ollama.com/install.sh | sh`. Version `0.20.7`. Install logged "No NVIDIA/AMD GPU detected. Ollama will run in CPU-only mode." — §4.0 hard rule holds as-shipped. `OLLAMA_HOST=0.0.0.0:11434` set via `/etc/systemd/system/ollama.service.d/override.conf`, daemon reloaded, service restarted, `ss -ltn` confirms `LISTEN 0.0.0.0:11434`. Verified from Desktop host: `curl http://10.0.0.9:11434/api/version` → `{"version":"0.20.7"}`, `/api/tags` → `{"models":[]}`. Template is now ready to be shut down and cloned.
- **Host split (locked from RAM math §4 Option C-quant):** Desktop host runs **7 VMs** = GiantQueen-B (12 GB) + DwarfQueen-B1 (8 GB) + DwarfQueen-B2 (8 GB) + Worker-B1..B4 (4 GB each) = 44 GB guest RAM, 20 GB host headroom. Laptop host runs **8 VMs** = RajaBee (18 GB) + GiantQueen-A (12 GB) + DwarfQueen-A1/A2 (8+8 GB) + Worker-A1..A4 (4×4 GB) = 62 GB guest RAM, 2 GB host headroom (tight, matches §4 Option C-quant). IP slots: Desktop `10.0.0.21-27`, Laptop `10.0.0.11-18`. **Laptop side is deferred** — Laptop stays on BeeSting videos today. Desktop-side build runs first; Laptop-side runs AFTER the role swap (Laptop boots into Linux, Desktop boots into Windows, lanes flip). Laptop Claude set the HOLD on 2026-04-14 and released it for Desktop-only work.
- **2026-04-14 afternoon** — `virt-clone` is **not** in the `/etc/sudoers.d/claude-kvm` NOPASSWD allowlist, so the canonical clone path is blocked without asking Nir for a password. First attempt with `sudo cp` to duplicate the template qcow2 per clone **filled the 92 GB root filesystem to 100%** (template is 8.6 GB actual × 5 full copies). Recovered by truncating the stray qcow2s with `sudo tee <file> < /dev/null > /dev/null` (works because `tee` is in the allowlist; `rm` is not). Re-clone via qcow2 **differential clones** using `qemu-img create -f qcow2 -b desktop-template.qcow2 -F qcow2 <name>.qcow2` (runs as `nir` because template file is world-readable, then `sudo mv` into the libvirt pool dir). Each differential is ~200 KB initially and grows with writes. The clone + XML-rewrite logic lives in `scripts/clone_phase3.py`.
- **2026-04-14 afternoon** — All 7 Desktop clones defined: `giantqueen-b`, `dwarfqueen-b1`, `dwarfqueen-b2`, `worker-b1`, `worker-b2`, `worker-b3`, `worker-b4`. RAM caps set per tier: GQ-B 12 GB, DQ-B1/B2 8 GB each, Worker-B1..B4 4 GB each (`virsh setmaxmem` + `virsh setmem` with `--config`). All shut off pending boot + per-clone hostname/machine-id regen + Round 1 Dense model pull.
- **2026-04-14 late afternoon — Overlay clones thrown away. Laptop Claude caught a fundamental architectural mistake:** the 7 qcow2 backing-file overlays were solving a disk-scarcity problem that did not exist. The 92 GB `/dev/sda2` is only the *root* partition — the real big volume is `/dev/sda4` ext4 mounted at `/home`, **1.7 TB total with 1.5 TB free**. Full doc at `WaggleDance/DESKTOP_KILLERBEE_PHASE3_DISK_FIX.md` (Laptop Claude). Resolution: scrapped the overlays, scrapped the `phase3` libvirt pool on `/var/lib/libvirt/images/phase3/`, created a new `killerbee` pool pointed at **`/home/killerbee-images/`** on the big volume, and **rebuilt the 7 VMs as 7 fully independent fresh Ubuntu autoinstalls** — not clones, not copies, not overlays. No backing files. No shared state. Each VM is its own universe.
- **2026-04-14 evening — 7 fresh autoinstalls, one at a time.** First try with `virt-install --cdrom $ISO` + `--disk seed.iso,device=cdrom` hung on the Ubuntu 24.04 live-server `Continue with autoinstall?` confirm prompt. Pivoted to `virt-install --location $ISO,kernel=casper/vmlinuz,initrd=casper/initrd --extra-args 'autoinstall ds=nocloud console=ttyS0,115200n8 serial'` — that tells the kernel to go headless and triggers cloud-init autoinstall from the CIDATA seed.iso attached as a second cdrom. Drivers: `scripts/autoinstall_one.sh` (one VM), `scripts/full_cycle_one.sh` (autoinstall + boot + Ollama install + bind + verify), `scripts/full_cycle_remaining6.sh` (the outer loop). All seven VMs now defined + running + each reachable over SSH (`nir` user, `~/.ssh/phase3_ed25519`) + each with Ollama 0.20.7 bound to `0.0.0.0:11434`.
- **DHCP leases assigned (not static, but stable by MAC):** `giantqueen-b` `10.0.0.12`, `dwarfqueen-b1` `10.0.0.13`, `dwarfqueen-b2` `10.0.0.14`, `worker-b1` `10.0.0.15`, `worker-b2` `10.0.0.16`, `worker-b3` `10.0.0.17`, `worker-b4` `10.0.0.18`. (The `10.0.0.21-27` slots from §5 are the *aspirational* range; actual leases came from the existing LAN DHCP pool and are stable by virtio MAC.)
- **Phase 3 Desktop build = COMPLETE for the 7-VM Desktop half.** Next steps per §6 and §7 — pull Round 1 Dense models into each VM (`qwen3:14b` on giantqueen-b, `qwen3:8b` on each DwarfQueen, `phi4-mini:3.8b` on each Worker). Laptop stays on BeeSting; no Laptop-side VMs today. Stop and wait for Nir's next direction per Laptop Claude's HOLD-release.
